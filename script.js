...
class Laser {
    ...
    render(context){
        ...
        if (this.game.spriteUpdated){
            this.game.waves.forEach(wave => {
                wave.enemies.forEach(enemy => {
                    if (this.game.checkCollisions(this, enemy)){
                        enemy.hit(this.damage);
                    }
                })
            })
            this.game.bossArray.forEach(boss => {
                if (this.game.checkCollisions(this, boss) && boss.y >=0){
...
class Enemy {
    ...
    update (x,y){
        ...
        this.game.projectilesPool.forEach(projectile => {
           if (!projectile.free && this.game.checkCollisions(this, projectile) && this.lives > 0){
                this.hit(1);
                projectile.reset();
...
class Boss {
    ...
    update(){
        this.speedY = 0;
        if(this.game.spriteUpdated && this.lives >= 1) this.frameX = 0;
        ...
        this.x += this.speedX;
        this.y += this.speedY;
        //check collision with projectiles for boss
        this.game.projectilesPool.forEach(projectile => {
            if(this.game.checkCollisions(this, projectile) && !projectile.free && this.lives >= 1 && this.y >= 0){
                this.hit(1);
                projectile.reset();
    ...
    //check collision with player
    if (this.game.checkCollisions(this, this.game.player) && this.lives >= 1){
        this.game.gameOver = true;
        this.lives = 0;
    }
    //boss destroyed
    if (this.lives < 1 && this.game.spriteUpdated){
        this.frameX++;
        if(this.frameX > this.maxFrame) {
            this.markedForDeletion = true;
...
class Game {
    ...
    getProjectile(){
        ...
        newWave(){
            ...
            this.wave = this.waves.filter(object => !object.markedForDeletion);
        }
        reset(){
            this.player.reset();
            this.columns = 2;
